...
const merge = (dupBlocks, l, mid, r) => {
    ...
    while(i <= mid){
        order.push([i, null, null, null])   //insert into new array
        arr.push(dupBlocks[i++])
    }

    while(j <= r){
        order.push([null, j, null, null])  //insert in new array
        arr.push(dupBlocks[j++])
    }
    
    for(i=l;i<=r;i++){
        dupBlocks[i] = arr[i - l]
        order.push([i, null, dupBlocks.slice(), null])   //sorted elements display
    }
